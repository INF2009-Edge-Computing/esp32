...
- MQTT topics are structured as `/commands/<NODE>/collect`,
  `/commands/<NODE>/thresholds`, `/sensors/<NODE>/status`.
...
- Thresholds storage on the dashboard is always a dict per node; lists are
...
- **Extending firmware**: edit `receiver.c` above the Wi‑Fi initialization, use
  ...
  already in place for thresholds and sessions.
...
1. **Copying files to the Pi** – use `scp` with the password `password`:
   ...
   ```
   Repeat for `server.py` or any other updated Python file.
